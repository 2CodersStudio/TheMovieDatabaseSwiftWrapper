...
public struct ListsMDB: ArrayObject{
  ...
  ///MARK: Lists
  public  static func lists(_ api_key: String!, listId: String!, completion: @escaping (_ clientReturn: ClientReturn, _ data: ListsMDB?) -> ()) -> URLSessionTask{
    let url  = "https://api.themoviedb.org/3/list/" + listId
    return Client.Lists(url, api_key: api_key, listId: listId!){
      apiReturn in
